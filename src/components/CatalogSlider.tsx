...
const CatalogSlider: React.FC<CatalogSliderProps> = ({ categories, catalogs }) => {
  ...
  const filteredCatalogs =
    selectedCategory === "ALL CATALOGUES"
      ? catalogs.filter((catalog) => categories.includes(catalog.category))
  ...
  return (
    <div className="bg-gray-700 text-white py-10 md:py-16 tracking-wide w-full overflow-hidden">
      ...
      {/* Swiper Slider or Centered Layout */}
      <div className="relative ml-32 md:mb-10">
        {filteredCatalogs.length <= 3 ? (
          <div className="flex justify-center gap-8 flex-wrap">
            {filteredCatalogs.map((catalog, index) => (
              <div
                key={index}
                className="w-full max-w-[160px] sm:max-w-[200px] md:max-w-[250px] flex flex-col items-center"
              >
                <img
                  src={catalog.image}
                  alt={catalog.title}
      ...
      {/* Navigation Arrows (Hidden in Centered Layout) */}
      {filteredCatalogs.length > 3 && (
